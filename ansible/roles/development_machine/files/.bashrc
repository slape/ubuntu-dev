...
term_colors() {
  ...
  PS1="${debian_chroot:+($debian_chroot)}$LIGHT_BLUE\W ~>$YELLOW\${prompt_job}$NO_COLOR"
  PS1="$LIGHT_BLUE\W ~> $YELLOW\${prompt_job}$NO_COLOR"
}
...
export EDITOR=$(basename $(command -v vim))
...
alias pl='git pull'
alias pu='git push'
alias dl="youtube-dl --download-archive archive.txt -i -x -f bestaudio --audio-format m4a --audio-quality 160k --embed-thumbnail --add-metadata -o '~/Music/%(playlist_index)s - %(title)s.%(ext)s'" 
alias ctf="docker run -it --rm --volume='wordlists:/wordlists:rw' --volume='ctf:/ctf:rw' kali-ctf"